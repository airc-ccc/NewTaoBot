...
'''
[["t", "77ca00e3299d5a708d3d1b6dfaf33116"], ["cookie2", "16f60fb312f749f01bbcfb10e5a26b96"], ["v", "0"], ["_tb_token_", "ee8eb77717917"], ["alimamapwag", "TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NDsgcnY6NTkuMCkgR2Vja28vMjAxMDAxMDEgRmlyZWZveC81OS4w"], ["cookie32", "6d50a6f9ad98f2c0f0a1978a4bf9cd5a"], ["alimamapw", "QhRcX15tCANSAwIEAQ4IUwIBDFZXVQQHCwEFBFZUAQheBQZSUAY%3D"], ["cookie31", "MjE4Njc3MDYsd3BobGosZ2VuZXI0MTJAc2luYS5jb20sVEI%3D"], ["login", "URm48syIIVrSKA%3D%3D"], ["cna", "w85PE4FQagYCAWU2iTPPrpDc"], ["isg", "BBsbLgCExWr51zmeHWvJoR-hqXyF8C_yadTDBQ1Y95ox7DvOlcC_QjnugsTiV4fq"]]
    ...
    #     create_user_info(msg, 0, tool)
        ...
        if patternURL.search(msg['Text']) == None:
            ...
            if (pattern_s.search(msg['Text']) != None) | (pattern_z.search(msg['Text']) != None) | (
                ...
                tburl = quote('http://tbyhq.ptjob.net/index.php?r=l&kw=' + msg['Text'][1:],
                              safe='/:?=&')
                text = '''
一一一一系统消息一一一一
...
邀请好友得返利：
http://t.cn/RnAKafe
                            ''' % (user_info[0][5], user_info[0][6], user_info[0][7], user_info[0][8], current_info, user_info[0][10], user_info[0][11], user_info[0][12], user_info[0][18], friends_count[0][0])
                cm.Close()
                itchat.send(text, msg['FromUserName'])
                return
...
# 创建用户账户
def create_user_info(msg, lnivt_code=0, tool=False, wxid=0, sourcname=0):
    cm = ConnectMysql()
    ...
    if lnivt_code == 0:
        sql = "INSERT INTO taojin_user_info(wx_number, sex, nickname, lnivt_code, withdrawals_amount, lnivter, create_time) VALUES('" + \
              res['NickName'] + "', '" + str(res['Sex']) + "', '" + res['NickName'] + "', '" + str(wxid) + "', '0.3', '" + str(lnivt_code) + "', '" + str(round(time.time())) + "');"
...
class WxBot(object):
    ...
    def add_friend(msg):
        ...
        create_user_info(msg, lnivt_code=sourc, tool=True, wxid=user_wxid, sourcname=sourcname)
        text = '''
一一一一 系统消息 一一一一
